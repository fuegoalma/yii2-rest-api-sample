...
| | |
| --- | --- |
| **The contract gates** | `config/openapi.yaml` is the published source of truth, and seven gates in [`tests/unit/contract/`](tests/unit/contract/) hold the code to it — routes in both directions, response schemas, search forms, write forms, RBAC, the published upload limits and the document's own integrity. Writing them found three real defects, including a document that was not valid YAML and a 255-character email address the API could never have accepted. See [ADR 5](docs/adr/0005-openapi-as-a-checked-contract.md). |
